...
def get_busy_stalls(mask_stalls, mask_detections, stalls_size, threshold=0.6):
...
def draw_parking(image, stalls, detections):
    ...
    for stall in stalls:
        ...
        cv2.fillPoly(stalls_mask, points,
                     stall_colors['busy'] if np.isin(stall_id, detections) else stall_colors['free'])
        ...
        cv2.putText(stalls_mask, str(stall_id), (center_poly[0] - text_wh[0][0], center_poly[1] + text_wh[0][1]),
